...
//   const [currentIndexJT, setCurrentIndexJT] = useState<any | null>(0);
...
//         {upCommingClassesdata?.map((_:any, index:any) => (
//           < View
//             key={index}
//             style={[
//               styles.indicator,
...
//       flatListRefJT.current?.scrollToIndex({  index: nextIndex });
//     }, 3000);
...
const UpCommingCarousel = ({ upCommingClassesdata }: any) => {
  ...
  useEffect(() => {
    // Ensure data is available before setting up the interval
    if (upCommingClassesdata && upCommingClassesdata.length > 0) {
      const intervalId = setInterval(() => {
        const nextIndex = (currentIndexJT + 1) % upCommingClassesdata.length;
        setCurrentIndexJT(nextIndex);
  ...
  }, [currentIndexJT, upCommingClassesdata]);
