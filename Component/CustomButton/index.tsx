...
const CustomButton = ({ onPress,onPressIn,opacity,disabled,onPressOut, btnTitle, color, backgroundColor, height, loading ,fontSize}: any) => {
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
      disabled={disabled}
      style={[styles.btn, { height: height ? height : 45 ,marginVertical: 0, backgroundColor: backgroundColor ? backgroundColor : Theme.darkGray,opacity: opacity ?opacity :'' }]}
    >
      {loading ? (
        <ActivityIndicator color={Theme.white} size="small" />
      ) : (
        <Text style={[styles.textType1, { color: color ? color : Theme.white, textAlign: 'center',fontSize: fontSize? fontSize:22 }]}>{btnTitle}</Text>
      )}
    </TouchableOpacity>
